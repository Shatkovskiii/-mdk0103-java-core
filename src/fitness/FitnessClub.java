...
public class FitnessClub {
    ...
    private Membership[] swimmingPoolMemberships;
    ...
    //ароверяем не просрочен ли абонемент если просрочен говорит что нельзя войти.
    private void enterZone(Membership[] zoneMemberships, Membership membership) {
        ...
        if (membership.getExpiryDate().isBefore(LocalDate.now()) ||
                !isAllowedForZone(membership, zoneMemberships)) {
            System.out.println("Невозможно пройти в данную зону по данному абонементу");
            return;
        }
        ...
        for (Membership membership : memberships) {
            if (membership != null) {
                ...
                System.out.println(customer.getFirstName() + " " + customer.getLastName() +
                        " (Год рождения: " + customer.getBirthYear() + ")");
            }
        }
    }
}